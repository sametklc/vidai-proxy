...
app.post("/video/generate_image", upload.single("image"), async (req, res) => {
  try {
    ...
    if (modelKeyLower === "runway") {
      ...
      if (model.needsFps24) input.fps = 24;
    } else if (modelKeyLower === "vidai") {
      // SeeDance (vidai) model requires image as string (URL)
      // Upload file to Replicate and get URL
      let imageUrl = null;
      try {
        console.log(`[IMAGE-TO-VIDEO] SeeDance model: Uploading image to Replicate...`);
        const uploadResponse = await fetch("https://api.replicate.com/v1/files", {
          method: "POST",
          headers: {
            "Authorization": `Token ${REPLICATE_API_TOKEN}`,
        ...
        const mimeType = req.file.mimetype || "image/jpeg";
        imageUrl = `data:${mimeType};base64,${base64}`;
        console.log(`[IMAGE-TO-VIDEO] Using data URI fallback for SeeDance model`);
      }
      ...
      if (model.needsFps24) input.fps = 24;
    } else if (modelKeyLower === "veo3") {
      // Veo3 model requires image as string (URL)
      // Upload file to Replicate and get URL
      let imageUrl = null;
      try {
        console.log(`[IMAGE-TO-VIDEO] Veo3 model: Uploading image to Replicate...`);
        const uploadResponse = await fetch("https://api.replicate.com/v1/files", {
          method: "POST",
          headers: {
            "Authorization": `Token ${REPLICATE_API_TOKEN}`,
        ...
        const mimeType = req.file.mimetype || "image/jpeg";
        imageUrl = `data:${mimeType};base64,${base64}`;
        console.log(`[IMAGE-TO-VIDEO] Using data URI fallback for Veo3 model`);
      }
